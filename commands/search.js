...
module.exports = {
    ...
    async execute(interaction) {
        ...
        if (!fetchedResults) return interaction.reply({ content: `No results found for \`${queryOption}\`.` });
        ...
        // Display a larger list of data if more than one user was found    
            ...
            if (fResults.length < 1500)
                await interaction.followUp({ content: `Showing **${userFetch.size}** members matching query \`${queryOption}\` (of ${limitOption.toString()} limit) regardless of cache.\n${fResults}` });
            else
                await interaction.followUp({ content: 'There were too many results! Please try and make your query field more specific.' });
        }
    },
};