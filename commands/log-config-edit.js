        ...
        LCONFIG.findOne({
            ...
            switch (configOption) {
                ...
                case "unignorechannel": {
                    ...
                    if (!data.ignoredchannels[ignoredChannelIndex]) return interaction.reply({ content: 'That category is not being ignored.' });
                ...
                case "deletedchannel":
                    ...
                    createWebhookButReuseIfPossible(interaction, configOption, data.deletechannel, channelOption);
                    break;
                case "editedchannel":
                    if (!channelOption) return interaction.reply({ content: 'This configuration value requires a `channel` option to be filled out.' });

                    createWebhookButReuseIfPossible(interaction, configOption, data.editchannel, channelOption);
                    break;
                case "usernamechannel":
                    if (!channelOption) return interaction.reply({ content: 'This configuration value requires a `channel` option to be filled out.' });
                    
                    createWebhookButReuseIfPossible(interaction, configOption, data.usernamechannel, channelOption);
                    break;
                case "vcchannel":
                    if (!channelOption) return interaction.reply({ content: 'This configuration value requires a `channel` option to be filled out.' });
                    
                    createWebhookButReuseIfPossible(interaction, configOption, data.vcchannel, channelOption);
                    break;
                case "updatechannel":
                    if (!channelOption) return interaction.reply({ content: 'This configuration value requires a `channel` option to be filled out.' });
                    
                    createWebhookButReuseIfPossible(interaction, configOption, data.chanupchannel, channelOption);
                    break;
                default:
                    break;
            }
...
function createWebhookButReuseIfPossible(interaction, config, oldchannel, channel) {
    LCONFIG.findOne({
        ...
        if (logChannels.some((lc) => {
            if (lc === channel.id) {
                const lcIndex = logChannels.indexOf(lc);
                if (lcIndex < 0) return interaction.reply({ content: 'Something went wrong.' });

                const indexedWebhook = logWebhooks[lcIndex];
                if (indexedWebhook === null) return interaction.reply({ content: 'Something went wrong.' });
